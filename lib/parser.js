...
proto.parseOne = function(transcript, cb) {
  ...
  return Promise.try(() => {
    ...
    if(this.settings.removeActions) {
      lines = _.map(lines, line => removeAll(line, this.regex.action));
    }
    if(this.settings.removeAnnotations) {
      //Remove annotations
      lines = _.map(lines, line => removeAll(line, this.regex.annotation));
    } else if(this.settings.removeTimestamps) {
      //Remove timestamps
      lines = _.map(lines, line => removeAll(line, this.regex.timestamp));
    }
